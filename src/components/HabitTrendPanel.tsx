...
import { detectHabitChanges } from '@/lib/habitDetection'
...
export default function HabitTrendPanel({
  history,
  currentBreakdown,
}: HabitTrendPanelProps) {
  const [summary, setSummary] = useState<string>(
    'Analyzing your recent habits...'
  )
  const insights = React.useMemo(() => {
    return history.length >= 2 ? detectHabitChanges(history) : []
  }, [history])
  ...
  useEffect(() => {
    if (history.length >= 2) {
      ...
      const fetchSummary = async () => {
        const text = await generateWeeklySummary(currentBreakdown, insights)
        setSummary(text)
      }
      fetchSummary()
    }
  }, [history, currentBreakdown, insights])
    ...
    return (
      <div className="w-full bg-slate-900 rounded-2xl p-8 border border-slate-800 shadow-xl text-center">
        ...
        <p className="text-slate-400">
          We need at least two footprint recordings over time to show trend
          analysis. Come back next month to see your habit trends!
        </p>
      </div>
    )
  }
    ...
    <div className="w-full space-y-6">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-200">
          Detected Habit Changes
        </h3>
        {insights.length === 0 ? (
          <p className="text-slate-400 text-sm">
            No significant changes detected in your specific categories
            recently.
          </p>
        ) : (
          <ul
            className="grid gap-4 md:grid-cols-2"
            aria-label="Detected habit changes"
          >
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span
                      className={`font-semibold capitalize ${isIncrease ? 'text-red-200' : 'text-emerald-200'}`}
                    >
