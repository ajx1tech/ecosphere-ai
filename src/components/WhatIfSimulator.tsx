...
import { CarbonProfile, FootprintBreakdown, WhatIfScenario } from '@/lib/types';
...
export default function WhatIfSimulator({ profile, currentBreakdown }: WhatIfSimulatorProps) {
  ...
  const chartData = [
    { year: 2026, ...Object.fromEntries(scenarios.map(s => [s.label, s.year2030])) },
    { year: 2030, ...Object.fromEntries(scenarios.map(s => [s.label, s.year2030])) },
    { year: 2040, ...Object.fromEntries(scenarios.map(s => [s.label, s.year2040])) },
    { year: 2050, ...Object.fromEntries(scenarios.map(s => [s.label, s.year2050])) }
  ];

  return (
    <div className="bg-slate-900 rounded-2xl p-6 md:p-8 border border-slate-800 shadow-xl">
      <h2 className="text-2xl font-bold text-slate-100 mb-6">What-If Simulator</h2>
      
      <div className="grid md:grid-cols-2 gap-8 mb-10">
        <div className="space-y-6">
          <SliderControl label="Diet Improvement (Plant-based shift)" value={dietShift} setValue={setDietShift} />
          <SliderControl label="Transport Shift (Greener commute)" value={transportShift} setValue={setTransportShift} />
          <SliderControl label="Shopping Reduction (Less stuff)" value={shoppingReduction} setValue={setShoppingReduction} />
          <SliderControl label="Energy Efficiency (Usage drop)" value={energyEfficiency} setValue={setEnergyEfficiency} />
        </div>
        ...
        <div className="h-64 md:h-full min-h-[300px]" aria-hidden="true">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <YAxis stroke="#94a3b8" width={60} tickFormatter={(val) => `${val}kg`} />
              <Tooltip 
                contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px' }}
                itemStyle={{ color: '#f8fafc' }}
              />
              <Legend wrapperStyle={{ paddingTop: '20px' }} />
              <Line type="monotone" dataKey="Business as Usual" stroke="#ef4444" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
              <Line type="monotone" dataKey="Moderate Changes" stroke="#eab308" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
              <Line type="monotone" dataKey="Full Transition" stroke="#10b981" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      {showTable && (
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300" aria-label="Simulation results table">
