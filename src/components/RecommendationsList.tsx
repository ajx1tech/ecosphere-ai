  ...
  return (
    <div className="w-full bg-slate-900 rounded-2xl p-6 md:p-8 border border-slate-800 shadow-xl">
      ...
      <p className="text-slate-400 text-sm mb-6">
        Personalized recommendations ranked by highest impact based on your
        footprint.
      </p>

      <ul
        className="space-y-4"
        aria-label="List of personalized recommendations"
      >
        <AnimatePresence>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div>
                    <h3
                      className={`font-semibold ${isCompleted ? 'text-slate-400 line-through' : 'text-slate-200'}`}
                    >
                      {rec.action}
                    </h3>
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full border ${getDifficultyColor(rec.difficulty)}`}
                      >
                        {rec.difficulty}
                      </span>
                    </div>
                ...
                <button
                  ...
                  aria-label={
                    isCompleted
                      ? `Completed: ${rec.action}`
                      : `Mark as done: ${rec.action}`
                  }
                >
