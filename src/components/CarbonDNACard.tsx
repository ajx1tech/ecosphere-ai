  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h2 className="text-sm font-bold text-emerald-400 tracking-wider uppercase mb-1">
            Your Archetype
          </h2>
        ...
        <div
          className={`px-4 py-2 rounded-full font-bold text-sm shadow-lg ${getRiskColor(dna.riskProfile)}`}
        >
      ...
      <div>
        <div className="flex justify-between items-end mb-3">
          ...
          <div className="text-sm text-slate-400 font-medium">
            Total: {breakdown.total.toLocaleString()} kg CO2e
          </div>
        </div>
        ...
        >
          <div
            ...
            title={`Transport: ${breakdown.transport} kg`}
          />
          <div
            style={{ width: `${dietPct}%` }}
            className="bg-green-500 hover:opacity-90 transition-opacity"
            title={`Diet: ${breakdown.diet} kg`}
          />
          <div
            style={{ width: `${shoppingPct}%` }}
            className="bg-purple-500 hover:opacity-90 transition-opacity"
            title={`Shopping: ${breakdown.shopping} kg`}
          />
          <div
            style={{ width: `${energyPct}%` }}
            className="bg-yellow-500 hover:opacity-90 transition-opacity"
            title={`Energy: ${breakdown.energy} kg`}
          />
          <div
            style={{ width: `${digitalPct}%` }}
            className="bg-teal-500 hover:opacity-90 transition-opacity"
            title={`Digital: ${breakdown.digital} kg`}
          />
        </div>

        {/* Legend */}
        <div
          className="flex flex-wrap gap-4 mt-4 text-sm text-slate-300"
          aria-hidden="true"
        >
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-blue-500"></span> Transport
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-green-500"></span> Diet
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-yellow-500"></span> Energy
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-teal-500"></span> Digital
          </div>
        </div>
      </div>
    </motion.div>
