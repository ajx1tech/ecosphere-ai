...
export default function GamificationPanel({
  ...
  useEffect(() => {
    try {
      ...
      const storedCompleted = JSON.parse(
        localStorage.getItem('ecosphere_missions_done') || '[]'
      )
      // eslint-disable-next-line react-hooks/set-state-in-effect -- Safe on mount to sync from localStorage
      setTotalXP(storedXP)
  ...
  const badges = checkBadgeEligibility(profile)
  ...
  return (
    <div className="w-full space-y-8 relative">
      {showConfetti && (
        <div
          className="absolute inset-0 pointer-events-none flex items-center justify-center z-50 overflow-hidden"
          aria-hidden="true"
        >
      ...
      <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800 shadow-xl flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="flex-1 w-full">
          ...
          <p className="text-slate-400 text-sm mb-4">
            Total XP:{' '}
            <span className="text-emerald-400 font-bold">{totalXP}</span>
          </p>
          ...
          <div className="space-y-2">
            ...
            >
              <Progress.Indicator
                ...
                style={{
                  transform: `translateX(-${100 - calculateProgress()}%)`,
                }}
              />
            </Progress.Root>
          </div>
              ...
              return (
                <motion.li
                  ...
                  className={`p-4 rounded-xl border flex items-center gap-4 transition-colors ${
                    isDone
                      ? 'bg-slate-800/50 border-emerald-900/30'
                ...
                >
                  <button
                    ...
                    className={`flex-shrink-0 w-6 h-6 rounded-md border flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 ${
                      isDone
                        ...
                        : 'bg-slate-900 border-slate-600 text-transparent hover:border-emerald-500'
                    }`}
                    aria-label={
                      isDone
                        ? `Completed quest: ${mission.title}`
                        : `Complete quest: ${mission.title}`
                    }
                  >
                    ✓
                  </button>
                  <div className="flex-1">
                    <div
                      className={`font-semibold text-sm ${isDone ? 'text-slate-500 line-through' : 'text-slate-200'}`}
                    >
                      {mission.title}
                    </div>
                  ...
                  <div
                    className={`font-bold text-sm ${isDone ? 'text-slate-600' : 'text-emerald-400'}`}
                  >
                    +{mission.xpReward} XP
                  </div>
                </motion.li>
