 ...
 */
export function detectHabitChanges(history: { profile: CarbonProfile; breakdown: FootprintBreakdown; timestamp: Date }[]): HabitChangeInsight[] {
  if (history.length < 2) {
  ...
  for (const { key, label } of categories) {
    ...
    if (Math.abs(changePercent) > 10) {
      ...
      if (label === 'shopping' && changePercent > 0) {
        reason = 'likely due to increased online orders or fast fashion purchases';
