...
export function calculateLevel(totalXP: number): {
  ...
  if (totalXP < 100)
    return { level: 1, title: 'Polluter 🏭', xpToNext: 100 - totalXP }
  if (totalXP < 300)
    return { level: 2, title: 'Aware Citizen 🌱', xpToNext: 300 - totalXP }
  if (totalXP < 600)
    return { level: 3, title: 'Climate Hero 🦸', xpToNext: 600 - totalXP }
...
export function generateWeeklyMissions(
  ...
  if (
    maxCategory === diet &&
    (profile.dietType === 'high-meat' || profile.dietType === 'moderate-meat')
  ) {
    missions.push({
    ...
    maxCategory === transport &&
    profile.transportMode === 'car-solo'
  ) {
    missions.push({
      ...
      estimatedSavingsKg: Math.round(
        profile.commuteDistanceKm * 2 * (0.21 - 0.04)
      ),
    })
 ...
 */
export function checkBadgeEligibility(profile: CarbonProfile): Badge[] {
  const badges: Badge[] = [
    {
      id: 'no_meat_monday',
      name: 'No Meat Monday',
    ...
    },
    {
      ...
      icon: '🚲',
      description:
        'Use public transit or zero-emission modes for your primary commute.',
      earned:
        profile.transportMode === 'public-transit' ||
        profile.transportMode === 'bike-walk',
    },
    {
      id: 'minimalist_month',
      ...
      icon: '📦',
      description:
        'Keep online shopping and fast fashion purchases to a minimum.',
      earned:
        profile.onlineShoppingPerMonth <= 1 &&
        profile.fastFashionPerMonth === 0,
    },
    {
      id: 'energy_saver',
      name: 'Energy Saver',
